...
function formatRange(date1, date2, formatStr, separator, isRTL) {
	...
	separator = separator || '-'; //change by whb -- remove blank for separator; separator = separator || ' - ';
...
$.extend(DayGrid.prototype, {
	...
	renderSegHtml: function(seg, disableResizing) {
		...
		if (!event.allDay && seg.isStart) {
		    timeHtml = '<span class="fc-time">' + htmlEscape(view.getEventTimeText(event)) + title + '</span>';
		}

		titleHtml =
			'<span class="fc-title">' +
				(htmlEscape(event.room || '') || '&nbsp;') + // we always want one line of height
			'</span>';
		var teacherColor = event.teacherColor || '';
		var teacherBgColor = '';
		
		if ('' != teacherColor) {
		    teacherBgColor = 'background-color:' + teacherColor;
		}

		var roomColor = event.roomColor || '';
		var roomBgColor = '';
		if ('' != roomColor) {
		...
		if ('' == conflict && 'courseevent' == eventclass) {
		    ...
		    if (conflict.indexOf('room') > -1) {
		        conflictDiv += '<div class="roomconflict"></div>';
		    }
		}
		...
		return '<a name="' + event.id + '" class="' + classes.join(' ') + lessonClass + eventfinish + '"' +
				...
				(skinCss ?
					' style="' + borderColor + skinCss + '"' :
					' style="' + borderColor  + '"'
					) +
			'>' +
				'<div class="fc-content" title="' + $(timeHtml).html() + $(titleHtml).html() + '">' + // change by whb, add title '<div class="fc-content">' +
					(isRTL ?
